...
function http (url, opt) {
  if (opt.jar && opt.jar.proxy) {
    console.log({
      host: opt.jar.proxy.split(';')[0].split(':')[0],
      port: opt.jar.proxy.split(';')[0].split(':')[1],
      // socks5 auth
      username: opt.jar.proxy.split(';')[1],
      password: opt.jar.proxy.split(';')[2],
    })
...
exports.func = function (args) {
  ...
  if (opt.jar) {
    if (typeof opt.jar.proxy === 'string') {
      opt.proxy = opt.jar.proxy
    }
  }
